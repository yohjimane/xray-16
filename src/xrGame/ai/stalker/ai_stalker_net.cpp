...
{
    return (inherited::net_SaveRelevant() || PPhysicsShell() != NULL);
}
...
void CAI_Stalker::net_Export(NET_Packet& P)
{
    ...
    else
    {
        ...
        P.w_float(movement().m_body.current.roll);
        ...
        auto torsoIdx = m_animation_manager->torso().animation().idx;
        auto torsoSlot = m_animation_manager->torso().animation().slot;
        P.w_u16(m_animation_manager->torso().animation().idx);
        P.w_u16(m_animation_manager->torso().animation().slot);
        P.w_u16(m_animation_manager->legs().animation().idx);
        P.w_u16(m_animation_manager->legs().animation().slot);
        P.w_u16(m_animation_manager->head().animation().idx);
        P.w_u16(m_animation_manager->head().animation().slot);

        P.w_u16(m_animation_manager->script().animation().idx);
        P.w_u16(m_animation_manager->script().animation().slot);
    }
}
...
void CAI_Stalker::net_Import(NET_Packet& P)
{
    ...
    else
    {
        ...
        u16 u_torso_motion_slot;
        u16 u_legs_motion_idx;
        u16 u_legs_motion_slot;
        u16 u_head_motion_idx;
        u16 u_head_motion_slot;
        u16 u_script_motion_idx;
        u16 u_script_motion_slot;

        P.r_float(f_health);
        P.r_vec3(fv_position);
        P.r_float(fv_direction.x);
        P.r_float(fv_direction.y);
        P.r_float(fv_direction.z);
        P.r_float(fv_head_orientation.x);
        P.r_float(fv_head_orientation.z);

        P.r_u16(u_active_weapon_slot);

        P.r_u16(u_torso_motion_idx);
        P.r_u16(u_torso_motion_slot);
        P.r_u16(u_legs_motion_idx);
        P.r_u16(u_legs_motion_slot);
        P.r_u16(u_head_motion_idx);
        P.r_u16(u_head_motion_slot);

        P.r_u16(u_script_motion_idx);
        P.r_u16(u_script_motion_slot);
